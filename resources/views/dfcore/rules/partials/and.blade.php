...
$box_msg =  trans('messages.rules_if_lbl24');
$box_type = 'warning';
...
?>
            ...
            <div class="box box-{!! $box_type !!} box-solid rules-if-field-container-{!! $if_id !!} ">
                ...
                <div class="box-body " style="display: block;">
                    ...
                    <div class="row">
                        <div class="col-lg-4">
                            ...
                            <select class="form-control if_field select2" name="if_field[{!! $if_id !!}]" data-if_identifier="{!! $if_id !!}">
                                ...
                                <option value="all" {{ $if_rules['if_field'][$if_id] == 'all' ? 'selected' : '' }}>All</option>
                    ...
                    <div class="row" style="margin-top: 20px; margin-bottom: 30px;">
                        ...
                        <div class="col-lg-8 if-button" style="margin-top:20px;">
                           ...
                           @if($operator === \App\DfCore\DfBs\Enum\RuleConditions::AND_OPERATOR)
                            <button type="button" class="btn btn-warning delete-if-field btn-xs"
                                    data-container="{!! $if_id !!}"
                                    data-operator="{!! \App\DfCore\DfBs\Enum\RuleConditions::AND_OPERATOR !!}"
                                    data-parent="{!! $parent !!}"><i class="fa fa-trash"></i> </button>
                            @endif

                                <button type="button"  id="or_button" style="{!! $if_rules['if_field'][$if_id] == 'all' ? 'display:none;'  : '' !!}"
                ...
                <div class="append_or_{!! $if_id !!} top_rules">
                    @foreach($rules_dictonary['rules']['if_operator'] as $if_id => $operator)
                        @if($operator === \App\DfCore\DfBs\Enum\RuleConditions::OR_OPERATOR)
                          @if(empty($rules_dictonary['rules']['if_main_parent'][$if_id]))
                            <?php $parent_id = $rules_dictonary['rules']['if_parent_child'][$if_id];?>
                                @if($parent == $parent_id)
                                    @include('dfcore.rules.partials.or',['if_id'=>$if_id,'spacer'=>'rules-spacer','field_names','if_rules'=>$if_rules,'parent'=>$parent_id])
                                @endif
                          @endif
                        @endif
                    @endforeach

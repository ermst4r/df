 ...
 */
class OOShopping extends AbstractChannel
            ...
            foreach($results as $generated_id=>$source) {
                ...
                foreach ($mapping_template as $mapping_value) {
                    switch ($mapping_value->channel_field_name) {
                        ...
                        default:
                            $item->addChild($mapping_value->channel_field_name,'<![CDATA['.$this->sanitizeSimpleXmlNodes($source['_source'][$mapping_value->channel_field_name]).']]>','http://base.google.com/ns/1.0');
                    }
                }

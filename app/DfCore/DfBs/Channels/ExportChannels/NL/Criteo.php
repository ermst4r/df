 ...
 */
class Criteo extends AbstractChannel
    implements iExportChannel
{
    ...
    protected function buildLegacyFeed($results, $mapping_template, $custom_fields = [], $repeating_node = 'products', $append_childs = [],$root_node=null)
    {

        $producten = new \SimpleXMLElement('<products></products>');
            ...
            foreach($results as $generated_id=>$source) {
                ...
                foreach ($mapping_template as $mapping_value) {
                    ...
                    if($mapping_value->channel_field_name == 'id') {
                        $item->addAttribute('id',$this->sanitizeSimpleXmlNodes($source['_source'][$mapping_value->channel_field_name]));
                    } else {
    ...
    {
        if($this->channel_feed->fk_channel_type_id == 28 ) {
            return  $this->buildFeed($this->feed_data,$this->mapping_template, $this->custom_fields,'item');
        } else {
            return  $this->buildLegacyFeed($this->feed_data,$this->mapping_template, $this->custom_fields,'product');
        }

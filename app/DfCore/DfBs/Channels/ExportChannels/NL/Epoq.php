 ...
 */
class Epoq extends AbstractChannel
    implements iExportChannel
{
    ...
    protected function buildFeed($results, $mapping_template, $custom_fields = [], $repeating_node = 'products', $append_childs = [],$root_node=null)
    {
        ...
        $rss = new \SimpleXMLElement('<'.$this->root_node.' version="2.0" xmlns:g="http://base.google.com/ns/1.0" xmlns:e="http://base.google.com/cns/1.0" xmlns:c="http://base.google.com/cns/1.0"></'.$this->root_node.'>');
            ...
            foreach($results as $generated_id=>$source) {
                ...
                foreach ($mapping_template as $mapping_value) {
                    switch ($mapping_value->channel_field_name) {
                        ...
                        default:
                            if(strpos($mapping_value->channel_field_name,'e:') !== false ) {
                                $item->addChild($mapping_value->channel_field_name,$this->sanitizeSimpleXmlNodes($source['_source'][$mapping_value->channel_field_name]),'http://base.google.com/cns/1.0');

                            }

                            if(strpos($mapping_value->channel_field_name,'g:') !== false ) {

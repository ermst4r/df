...
/*
  |--------------------------------------------------------------------------
  | Calculate the sum between multiple fields
  |--------------------------------------------------------------------------
  */
...
class CalculateSumStrategy implements iContract
{
    /**
    ...
    public function handle($es_record, $then_field, $then_field_values,$then_spacing)
    {
        ...
        $sum_fields =  (isset($then_field_values[0]) ? $then_field_values[0] : '');
        $sum_value = 0.00;
        if(isset($es_record['_source'][$then_field])) {
            foreach($sum_fields as $sum_f) {
                if(isset($es_record['_source'][$sum_f])) {
                    $sum_value+= $es_record['_source'][$sum_f];
                }
            }
        }
        $es_record['_source'][$then_field] = number_format($sum_value,2,'.','');
        return $es_record;
    ...
    {
        return RuleConditions::THEN_CALCULATE_SUM;
    }

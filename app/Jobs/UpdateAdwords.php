...
class UpdateAdwords implements ShouldQueue
{
    ...
    public function failed(\Exception $e)
    {
        ...
        $this->task = $this->task_log->createTask(['fk_feed_id'=>$this->feed_id,'task'=>'Importing Adwords feed '.$this->get_adwords_feed->name .' failed','status'=>TasklogEnum::FAILED]);
        $this->adwords_feed->createAdwordsFeed(['updating'=>false,'Pushing to adwords was interupted due the following error: '. $e->getMessage()],$this->fk_adwords_feed_id);
    }
     ...
     * @param $feed_id
     */
    public function __construct($fk_adwords_feed_id,$feed_id,$update_all,$preview)
    {
        //
        ...
        $this->importfeed_facade = new ImportFeedFacade();
        $this->task = $this->task_log->createTask(['fk_feed_id'=>$this->feed_id,'task'=>'Importing Adwords '.$this->get_adwords_feed->name,'status'=>TasklogEnum::BUSY]);
    }
    ...
    public function handle()
    {
         ...
         * Globa; settings
         */
        $fk_adwords_feed_id = $this->fk_adwords_feed_id;
        ...
        $this->adwords_feed->createAdwordsFeed(['updating'=>false],$this->fk_adwords_feed_id);
        $this->task_log->createTask(['status'=>TasklogEnum::FINISHED,'task'=>'Imported Adwords feed '.$this->get_adwords_feed->name,'fk_feed_id'=>$this->feed_id],$this->task->id);
        event(new \App\Events\AdwordsPreviewCreated($fk_adwords_feed_id));
    }
}
